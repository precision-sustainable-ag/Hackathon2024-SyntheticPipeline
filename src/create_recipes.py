...
class RecipeCreator:
    ...
    def create_synthetic_image(self, background_image_id: str) -> Dict[
        str, Any]:
        """
        ...
        """
        synthetic_image_id = str(
            uuid.uuid4())  # Generate a unique ID for synthetic image
    ...
    def add_cutout_to_image(self, synthetic_image: Dict[str, Any],
        ...
        max_cutouts_per_image = random.randint(
            self.cfg.cutout_filters.cuts_n_image['min'],
            self.cfg.cutout_filters.cuts_n_image['max'])
        ...
        if len(synthetic_image['cutouts']) < max_cutouts_per_image:
            ...
            log.debug(
                f"Added cutout {cutout.get('cutout_id')} to synthetic image {synthetic_image['synthetic_image_id']}.")
        else:
            log.debug(
                f"Reached max cutouts for synthetic image {synthetic_image['synthetic_image_id']}.")
...
class MongoDBRecipeManager:
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.collection = self.db[
            cfg.mongodb.collection]  # Access MongoDB collection
        self.recipe_creator = RecipeCreator(cfg)
        self.output_dir = Path(cfg.paths.projectdir,
                               "recipes")  # Output directory for synthetic image recipes
        self.output_dir.mkdir(parents=True, exist_ok=True)

    def weighted_sampling(self, documents: List[Dict[str, Any]]) -> List[
        Dict[str, Any]]:
        """
        ...
        for class_name, group in class_groups.items():
            weight = weights.get(class_name,
                                 1.0)  # Default weight of 1 if not in config
            weighted_population.extend(
                group * int(weight * 10))  # Adjust multiplier as needed
    ...
    def process_cutouts(self, documents: List[Dict[str, Any]]) -> None:
        ...
        background_images = list(
            Path(self.cfg.paths.backgrounddir).glob('*.JPG')) + list(
            Path(self.cfg.paths.backgrounddir).glob('*.jpg'))
        ...
        required_cutouts = total_images * self.cfg.cutout_filters.cuts_n_image[
            'max']
        ...
        if len(documents) < required_cutouts and not self.cfg.cutout_filters.reuse_cutouts:
            log.warning(
                "Not enough documents to fulfill the required cutouts without reuse.")
        ...
        for image_index in range(total_images):
            ...
            num_cutouts_for_image = random.randint(
                self.cfg.cutout_filters.cuts_n_image['min'],
                self.cfg.cutout_filters.cuts_n_image['max'])
            ...
            if len(available_cutouts) < num_cutouts_for_image and not self.cfg.cutout_filters.reuse_cutouts:
                log.warning(
                    f"Not enough cutouts for image {image_index + 1}, using {len(available_cutouts)} instead.")
                sampled_cutouts = available_cutouts  # Use all available cutouts
            elif len(available_cutouts) < num_cutouts_for_image:
                log.warning(
                    f"Not enough cutouts for image {image_index + 1}, reusing cutouts.")
                sampled_cutouts = available_cutouts  # Use all available cutouts
                sampled_cutouts = random.choices(available_cutouts,
                                                 k=num_cutouts_for_image)
            else:
                sampled_cutouts = random.sample(available_cutouts,
                                                num_cutouts_for_image)  # Randomly sample the required number of cutouts
            ...
            synthetic_image = self.recipe_creator.create_synthetic_image(
                background_image_id)

            # Add each sampled cutout to the synthetic image
            for cutout in sampled_cutouts:
                if not self.cfg.cutout_filters.reuse_cutouts and cutout[
                    "_id"] in self.recipe_creator.used_cutouts:
                    continue  # Skip cutout if it has already been used and reuse is not allowed
                self.recipe_creator.used_cutouts.add(
                    cutout["_id"])  # Track used cutouts
                self.recipe_creator.add_cutout_to_image(synthetic_image,
                                                        cutout)  # Add cutout to the synthetic image
            ...
            if not self.cfg.cutout_filters.reuse_cutouts:
                available_cutouts = [doc for doc in available_cutouts if doc[
                    "_id"] not in self.recipe_creator.used_cutouts]

            # Log the creation of the synthetic image if any cutouts were added
            if cutouts_added_to_image:
                log.info(
                    f"Generated {image_index + 1} / {total_images} images.")
            else:
                log.warning(
                    f"No cutouts added for image {image_index + 1}. Skipping.")

        # Save all the generated recipes to JSON
        self.recipe_creator.save_recipes(self.output_dir)
        log.info(
            f"Total images generated: {min(image_index + 1, total_images)}")
...
def main(cfg: DictConfig) -> None:
    ...
    """
    log.info(
        "Starting recipe creation process.")  # Log the start of the process
